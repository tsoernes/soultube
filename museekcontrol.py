...
try:
    opts, args = getopt.getopt(sys.argv[1:], "hc:vi:p:j:l:p:m:b:tar", [
        ...
        "monitor-transfers", "info=", "minfo", "browse=", "sprivates",
        "sprivate=", "leave=", "join=", "joined", "sroom=", "stalk", "rooms",
        "bs=", "bsearch=", "rs=", "rsearch=", "gs=", "alltickers", "tickers=",
        "gsearch=", "version", "log=", "chat=", "pm=", "private=", "message=",
        "away", "download=", "mt", "mtransfers", "mb", "mbrowse", "buddy=",
        "unbuddy=", "ignore=", "unignore=", "ban=", "unban=", "trust=",
        "distrust=", "comment=", "roomlist", "abortdown=", "abortup=",
        "retryup=", "retrydown=", "removeup=", "removedown=", "upload=",
        "roominfo=", "roomsinfo", "ip=", "iproom=", "setticker=",
        "settempticker=", "setdefaultticker", "reloadshares", "disconnect",
        "connect"
    ])
...
for opts, args in opts:
    ...
    elif opts in ("--abortdown", "--abortup", "--removedown", "--removeup",
                  "--retryup", "--retrydown", "--upload"):
        if checkUrl:
            want = opts[2:]
...
class museekcontrol(driver.Driver):
    ...
    def process(self):
        ...
        if self.connected == 1:
            ...
            if want == "say":
                ...
                sys.exit()
            elif want in ("stalk", "rooms", "sprivate", "sprivates",
                          "transfers", "mtransfers", "joined", "tickers"):
                pass
                ...
                if message is not None:
                    self.send(
                        messages.ConfigSet("default-ticker", "ticker",
                                           message))
                sys.exit()
            elif want == "away":
                pass
                ...
                self.send(messages.TransferAbort(1, user, ufile))
                sleep(1)
                sys.exit()
    ...
    def cb_room_state(self, roomlist, joined, tickers):
        ...
        if want == "rooms":
            ...
            if room is not None:
                print "Size-----Name-------------------------------------------------------"
                print "%s\t%s" % (roomlist[room], room)
                print "Users" + (
                    "-" * 25) + "Status----Speed---Down----Files---Dirs"
                r = joined[room]
                list = joined[room].keys()
                list.sort()
                for u in list:
                    z = str(self.status_check(int(r[u][0]))) + "\t" \
                            + str(r[u][1]) + "\t" + str(r[u][2]) + "\t" \
                            + str(r[u][3]) + "\t" + str(r[u][4])
                ...
                for s in x:
                    ...
                    r = joined[s]
                    for u in joined[s]:
                        z = str(self.status_check(int(r[u][0]))) + "\t" + str(
                            r[u][1]) + "\t" + str(r[u][2]) + "\t" + str(
                                r[u][3]) + "\t" + str(r[u][4])
                        print "%s%s%s" % (u, " " * (30 - len(u)), z)
                    print ""
    ...
    def cb_room_said(self, roomname, user, text):
        ...
        if good == 1:
            if text[:4] == "/me ":
                ...
                message = "[" + user + "] " + text
            print "%s (%s) %s" % (time.strftime("%d %b %Y %H:%M:%S"), roomname,
                                  message)
    ...
    def cb_private_message(self, direction, timestamp, username, message):
        ...
        if want == "sprivates" or want == "sprivate":
            ...
            if doit == 1:
                if direction == 0:
                    print "(PM)", time.strftime("%d %b %Y %H:%M:%S"), \
                            username + " said: " + message
                elif direction == 1:
                    print "(PM)", time.strftime("%d %b %Y %H:%M:%S"), \
                            "You told", username + ": " + message
    ...
    def display_shares(self, username, shares):
        ...
        for dirs, files in shares.items():
            ...
            for filez, stats in files.items():
                ...
                number = " " * (len_shares - len(str(self.browse_number))
                                ) + str(self.browse_number)

                if ftype in ("mp3", "ogg", "MP3", "OGG"):
                    output('[' + str(number) + '] ' + "Size: " + str(size) +
